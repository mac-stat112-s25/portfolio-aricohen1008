...
1. What is the pattern / trend of `elevation` of hiking trails? #the pattern is that the elevation of the mountains is decreasing
2. What is the relationship between a hike's `elevation` and typical `time` it takes to summit / reach the top?
 #there is no significant relationship between the elevation and the time to ascend the mountain
:::
...
It would be a lot more difficult to understand the data without being able to vizualize it
...
We would like the catergorical diffuculty varialbe to compare the height, steepness and time to ascend 
b.  What about a visualization of the *quantitative* `elevation` variable?
The elevation of the mountains are decreasing
...
1. How many hikes fall into each category? 8 difficult, 11 easy, and 27 moderate hikes
2. Are the hikes evenly distributed among these categories, or are some more common than others?
there are more moderate hikes than the other two categories
...
-   What, in general, is the first argument of the `ggplot()` function? the hikes (the survey)
-   What is the purpose of writing `x = rating`? to show which axis it should be on
-   What do you think `aes` stands for?!?
average, easy, strenous?
...
ggplot(hikes, aes(x = rating)) +
  geom_bar()
```
adding the geom bar introduces the bar chart
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  geom_bar() +
...
```
this gives labels the x and y axis
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  geom_bar(fill = "blue") +
...
ggplot(hikes, aes(x = rating)) +
  ...
  labs(x = "Rating", y = "Number of hikes")
```
this gives the values a orange outline to make it more aesthetic 
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  geom_bar(color = "orange", fill = "blue")  +
  ...
  theme_minimal()
```
this makes the backround a bit more minimalistic
...
-   What's the purpose of the `+`? When do we use it? the purpose is to add another component to the command ggplot
-   We added the bars using `geom_bar()`? Why "geom"? because geom is geometrical 
-   What does `labs()` stand for? labs helps us label the x and y axis
-   What's the difference between `color` and `fill`? fill fills the bar and color colors the outside of the bar

#### Part b {.unnumbered}

In general, bar charts allow us to examine the following properties of a *categorical* variable:

-   **observed categories**: What categories did we observe? we observed the difficulty in hikes
-   **variability between categories**: Are observations evenly spread out among the categories, or are some categories more common than others? moderate hikes was the most common category 

We must then *translate* this information into the *context* of our analysis, here hikes in the Adirondacks. Summarize below what you learned from the bar chart, in context.

I learned that out of the 46 Adirondack mountains, the hike to ascend them was seperated in different levels of difficulty, with 27 being moderate, eight being difficult, and 11 hikes being easy. 
#### Part c {.unnumbered}

Is there anything you don't like about this barplot? For example: check out the x-axis again.

The bar plot stops at 20 when the # of moderate hikes is larger than the that. I would increase the range of the bar chart to 30 hikes instead of 20.

### Exercise 6: Sad Bar Chart {.unnumbered}

Let's now consider some research questions related to the *quantitative* `elevation` variable:

1. Among the hikes, what's the *range* of elevation and how are the hikes *distributed* within this range (e.g. evenly, in clumps, "normally")? the range of elevation is 5344ft-3820ft the elevations are evenly distributed
2. What's a *typical* elevation? around 4400 ft
3. Are there any *outliers*, i.e. hikes that have unusually high or low elevations? the highest elevation change hike is 4500ft and the lowest is 1844 ft
...
ggplot(hikes, aes(x = elevation)) +
  ...
  theme_minimal()
```
This might not be the most effective form of visualization because it is on a large scale so it is hard to know the exact value of the graph and also it is not vizually appealing to look at and understand. 
