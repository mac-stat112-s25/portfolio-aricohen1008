...
A lot of states have students with average scores of 900 and alot of states have average scores of around 1050, this is an interesting shaped which isn't really an parabolic shape which is what I expected to see. 
...
ggplot(education, aes(y=sat, x=expend)) + geom_point() + geom_smooth(method="lm")
...
there is not really a relationship between sat scores and spending, I am a bit suprised but not completely suprised because sat scores can be based on how much tutoring someone has gotten, not their success in life
...
states that dont spend as much per kid and have higher sat scores tend to produce kids with lower salaries than states that spend more on education per kid and have lower sat scores
...
```{r}
ggplot(education, aes(y=sat, x=expend, fill=fracCat))+ 
  geom_point() +
geom_smooth(method="lm")
...
because the states with higher sat scores have lower rates of participation, so even though the states spend less and get higher scores overall, that is because the students who do take the sat are better set up to take it that in other states where the participation rate is far higher, and more students, regardless of how set up they are for the sat, are taking it. 
